...
def classifier(mat, model):
    ...
    '''
    if model=='SVM':
        model = pickle.load(open("classification_model_1.pkl", "rb"))
        pred = model.predict(mat)
        
    elif model=='RF':
        model = pickle.load(open("classification_model_2.pkl", "rb"))
        pred = model.predict(mat)
        
    elif model=='GBC':
        model = pickle.load(open("classification_model_3.pkl", "rb"))
        pred = model.predict(mat)
